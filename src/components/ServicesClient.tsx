...
export default function ServicesClient() {
  ...
  return (
    <div className="bg-[#FAFBFC] min-h-screen font-sans">
      ...
      <section className="relative py-20 lg:py-28 px-4 sm:px-6 lg:px-8 border-b border-[#E2E8F0] overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto text-center space-y-5 relative z-10 min-h-[220px]">
          ...
          <div className="pt-2 flex flex-wrap items-center justify-center gap-3">
            <Link
              ...
              className="inline-flex items-center gap-2 bg-gradient-to-r from-[#C45B2A] to-[#E65100] hover:from-[#A34920] hover:to-[#C45B2A] text-white font-bold text-xs sm:text-sm px-6 py-3 rounded-full shadow-md shadow-orange-500/25 transition-all duration-200 hover:scale-[1.02] active:scale-[0.98] cursor-pointer min-h-[44px]"
            >
              <Package className="w-4 h-4" />
              <span>{t("servicesPage.ctaShip")}</span>
            </Link>
            <Link
              href={getLocalizedPath("/track")}
              className="inline-flex items-center gap-2 bg-white/95 hover:bg-white text-gray-900 border border-gray-200/90 font-bold text-xs sm:text-sm px-6 py-3 rounded-full shadow-2xs hover:border-[#C45B2A]/40 transition-all duration-200 hover:scale-[1.02] active:scale-[0.98] cursor-pointer min-h-[44px]"
            >
              <Search className="w-4 h-4 text-[#C45B2A]" />
              <span>{t("servicesPage.ctaTrack")}</span>
            </Link>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-16 lg:py-24">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {services.map((s, idx) => (
            <div
              key={idx}
              className={`bg-white/95 backdrop-blur-md rounded-[28px] border border-slate-200 overflow-hidden shadow-[0_10px_30px_rgba(15,23,42,0.05)] hover:shadow-[0_20px_40px_rgba(196,91,42,0.12)] hover:border-slate-300 transition-all duration-300 flex flex-col group ${
            ...
            >
              ...
              <div className="h-56 w-full bg-gray-100 overflow-hidden relative">
                ...
                <div
                  ...
                  } w-11 h-11 rounded-2xl bg-white/95 backdrop-blur-md text-[#C45B2A] border border-white/60 flex items-center justify-center shadow-md z-10`}
                >
                  {s.icon}
                </div>
              </div>
              ...
              <div className="p-6 sm:p-7 space-y-4 flex-grow flex flex-col justify-between">
                ...
                <div className="pt-5 border-t border-gray-100 flex items-center justify-between gap-2">
                  <Link
                    href={getLocalizedPath(`/services/${s.slug}`)}
                    className="text-xs font-bold text-gray-900 hover:text-[#C45B2A] inline-flex items-center gap-1.5 group/link transition-colors cursor-pointer"
                  >
                  ...
                  <Link
                    href={getLocalizedPath(`/services/${s.slug}`)}
                    className="px-3.5 py-1.5 rounded-full bg-orange-50 text-[#C45B2A] hover:bg-[#C45B2A] hover:text-white text-[11px] font-bold transition-colors cursor-pointer"
                  >
                    {t("servicesPage.requestQuoteBtn")}
                  </Link>
                </div>
              </div>
            </div>
      ...
      <section className="bg-white border-t border-b border-gray-200/80 py-16 px-4 sm:px-6 lg:px-8">
        <div className="max-w-[1400px] mx-auto space-y-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            <div className="bg-[#F8FAFC] p-6 rounded-2xl border border-slate-200 space-y-2 text-center">
        ...
        <div className="max-w-[1400px] mx-auto bg-gradient-to-r from-brand-dark-deep via-brand-dark to-brand-dark-deep rounded-[32px] p-8 sm:p-12 text-white text-center space-y-6 relative overflow-hidden shadow-2xl border border-brand-dark-border">
          ...
          <div className="flex flex-wrap items-center justify-center gap-4 relative z-10">
            ...
            <Link
              ...
              className="bg-white/10 hover:bg-white/20 text-white border border-white/20 font-bold text-xs sm:text-sm px-8 py-3.5 rounded-full backdrop-blur-md transition-all hover:scale-105 active:scale-95 cursor-pointer min-h-[44px]"
            >
              {t("contact.tag")}
            </Link>
          </div>
