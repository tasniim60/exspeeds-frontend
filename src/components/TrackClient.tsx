...
function TrackContent() {
  ...
  useEffect(() => {
    if (initialAwb) {
      ...
      const carrierKey = getCarrierKey(initialCarrierParam);
      setSelectedCarrier(carrierKey);
      executeTracking(carrierKey, clean);
    }
  ...
  const executeTracking = (carrierKey: string, rawAwb: string) => {
    ...
    if (!clean) {
      setValidationError(
        t("track.validationError") ||
          (isRTL
            ? "يرجى إدخال رقم بوليصة الشحن (AWB) بشكل صحيح."
            : "Please enter a valid tracking or AWB number.")
      );
      return;
    }
  ...
  const selectedCarrierObj =
    CARRIERS.find((c) => c.id === selectedCarrier) || CARRIERS[0];
      ...
      <section className="relative py-20 lg:py-28 px-4 sm:px-6 lg:px-8 border-b border-[#E2E8F0] overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto text-center space-y-5 relative z-10">
          ...
          <SectionEyebrow centered>
            {t("track.tag") ||
              (isRTL
                ? "التتبع المباشر لجميع الشحنات"
                : "Real-Time Cargo & AWB Tracking")}
          </SectionEyebrow>
          ...
          <p className="text-gray-600 text-sm sm:text-base lg:text-lg max-w-2xl mx-auto leading-relaxed font-medium">
            {t("track.subtitle") ||
              (isRTL
                ? "تأكيد فوري لحالة الشحنات الدولية والمحلية عبر أكثر من 12 شركة عالمية وشبكة XSPEED Express."
                : "Live tracking console for regional and global consignments across SMSA, DHL, FedEx, Aramex, UPS, TNT, XSPEED, DB Schenker, Air Cargo, Post/EMS, Container, and B/L.")}
          </p>
        </div>
      </section>

      {/* 2. Main Tracking Console */}
      <section className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12 lg:py-16 space-y-8">
        {/* Console Box */}
          ...
          <div className="space-y-3.5">
            <div className="flex items-center justify-between">
              <label className="block text-xs font-bold uppercase tracking-wider text-gray-700">
                {t("track.carrierSelectLabel") ||
                  (isRTL ? "اختر شركة الناقل" : "Select Express Carrier")}
              </label>
            ...
            {/* Direct Select Dropdown for all carriers */}
            <div className="pt-1">
              <select
            ...
            <div className="flex flex-col sm:flex-row gap-3 items-stretch">
              <div className="relative flex-1">
                <input
                  ...
                  placeholder={
                    selectedCarrierObj.placeholder ||
                    t("track.placeholder") ||
                    (isRTL
                ...
                <Search
                  ...
                  } top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400 pointer-events-none`}
                />
              </div>
        ...
        {/* 3. 4-Step Milestone Stepper Preview */}
        <div className="bg-white/95 backdrop-blur-md rounded-[32px] p-6 sm:p-8 md:p-9 border border-orange-100/90 shadow-sm space-y-6 text-start">
          <div className="space-y-1">
            ...
            <h3 className="text-xl sm:text-2xl font-display font-black text-gray-950">
              {isRTL
                ? "كيف تتم متابعة شحنتك خطوة بخطوة"
                : "How XSPEED Tracks Consignments Step-by-Step"}
            </h3>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3.5 sm:gap-4">
            <div className="bg-[#F8FAFC] p-4 sm:p-5 rounded-2xl border border-slate-200 space-y-2 hover:border-slate-300 transition-colors">
              ...
              <p className="text-xs text-gray-500 font-medium leading-relaxed">
                {isRTL
                  ? "مسح البار كود وإدخال البيانات بالمنظومة"
                  : "Scanned at origin facility & waybill generated"}
              </p>
            </div>
            ...
            <div className="bg-[#F8FAFC] p-4 sm:p-5 rounded-2xl border border-slate-200 space-y-2 hover:border-slate-300 transition-colors">
              ...
              <p className="text-xs text-gray-500 font-medium leading-relaxed">
                {isRTL
                  ? "مغادرة المطار أو الميناء إلى بلد الوصول"
                  : "Dispatched on active flight or sea vessel"}
              </p>
            </div>
            ...
            <div className="bg-[#F8FAFC] p-4 sm:p-5 rounded-2xl border border-slate-200 space-y-2 hover:border-slate-300 transition-colors">
              ...
              <p className="text-xs text-gray-500 font-medium leading-relaxed">
                {isRTL
                  ? "معالجة المستندات والإفراج الجمركي"
                  : "Nafeza & customs inspection finalized"}
              </p>
            </div>

            <div className="bg-[#F8FAFC] p-4 sm:p-5 rounded-2xl border border-slate-200 space-y-2 hover:border-emerald-200 transition-colors">
              ...
              <p className="text-xs text-gray-500 font-medium leading-relaxed">
                {isRTL
                  ? "تسليم الشحنة للعميل وتأكيد الاستلام"
                  : "Dispatched to recipient address with POD"}
              </p>
            </div>
          </div>
        </div>
