    ...
    <div className="bg-[#FAFBFC] min-h-screen font-sans">
      ...
      <section className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-16 lg:py-24 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
        ...
        <div className="lg:col-span-6 relative">
          <div className="relative rounded-[32px] overflow-hidden shadow-2xl border border-slate-200 bg-gray-100 h-[420px] sm:h-[480px]">
            <Image
          ...
          <div className="bg-gradient-to-r from-slate-50 via-white to-slate-50/80 rounded-[28px] p-6 sm:p-8 border border-slate-200 shadow-sm flex flex-col md:flex-row items-center gap-6">
            ...
            <div className={`space-y-1.5 flex-1 ${isRTL ? "text-right" : "text-left"}`}>
              <h3 className="text-lg font-bold text-gray-950">
                {isRTL
                  ? "الهوية التجارية والبوابة الرقمية المعتمدة (exspeeds.com)"
                  : "Brand Identity & Official Digital Portal (exspeeds.com)"}
              </h3>
              <p className="text-xs sm:text-sm text-gray-600 leading-relaxed font-medium">
                {isRTL
                  ? "تُعد منصة exspeeds.com هي البوابة التكنولوجية والمنظومة الرقمية الرسمية الحصرية لشركة إكس سبيد لخدمات الشحن السريع واللوجستيات (XSPEED Express Logistics). نقوم من خلال هذه البوابة بإدارة وتتبع الشحنات الدولية والمحلية، والتخليص الجمركي بمطار القاهرة والموانئ المصرية، وربط العملاء بالشبكة اللوجستية العالمية بأعلى معايير الأمان والدقة."
                  : "The exspeeds.com platform is the official, exclusive digital portal of XSPEED Express Logistics. Through this system, we orchestrate global freight forwarding, customs clearance at Cairo Airport and Egyptian seaports, real-time shipment tracking, and end-to-end supply chain logistics under verified international standards."}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-[1400px] mx-auto space-y-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {coreValues.map((val, idx) => (
              <div
                key={idx}
                className="bg-[#F8FAFC] rounded-[24px] p-6 sm:p-7 border border-slate-200 shadow-2xs hover:shadow-md hover:border-slate-300 transition-all duration-200 flex flex-col justify-between space-y-4 text-start"
              >
